...
class LLCRank(object):
    ...
    def start_critiquing(self):
        ...
        self.matrix_Test[self.matrix_Train>0] = 0

        users, items = np.nonzero(self.matrix_Test)
        print(len(items))
        self.item_keyphrase_freq[self.item_keyphrase_freq<20] = 0
        item_keyphrase_matrix = self.item_keyphrase_freq > 0
        print(np.sum(item_keyphrase_matrix))
        pos = (item_keyphrase_matrix.sum(axis=1) >= 5).nonzero()[0]
        print(len(pos))
        ...
        users = users[mask]
        t = 0
        for user, item in tqdm(zip(users, items),total=len(items)):
            t += 1
            if t > 5000:
                break
        ...
        #for user in tqdm(self.test_users):
            ...
            target_rank = self.target_ranks[0]
...
#                    print("The number of remaining_keyphrases is {}. remaining_keyphrases are: {}".format(len(remaining_keyphrases), remaining_keyphrases))
            ...
            # print(len(remaining_keyphrases))
            ...
            self.row['item_score'] = self.prediction_scores[user][item]
            ...
            for iteration in range(self.max_iteration_threshold):
                ...
                # print(max(prediction_scores_u), min(prediction_scores_u))
